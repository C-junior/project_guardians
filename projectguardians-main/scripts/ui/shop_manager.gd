...
@onready var merge_button: Button = $Control/Panel/MarginContainer/VBoxContainer/Footer/MergeButton
...
func _on_merge_pressed() -> void:
	# Hide shop temporarily and enter evolution mode
	visible = false
	EvolutionManager.start_evolution_selection()
	
	# When evolution completes or cancels, show shop again
	EvolutionManager.evolution_completed.connect(_on_evolution_done, CONNECT_ONE_SHOT)
	EvolutionManager.evolution_cancelled.connect(_on_evolution_done, CONNECT_ONE_SHOT)


func _on_evolution_done(_result = null) -> void:
	visible = true
	_update_merge_button()
...
func _update_merge_button() -> void:
	...
	# Check if any statues can be merged (using inventory + placed counts)
	var mergeable = EvolutionManager.get_mergeable_statue_types()
	var can_merge_any = mergeable.size() > 0
	
	merge_button.disabled = not can_merge_any
	if can_merge_any:
		var first = mergeable[0]
		var counts = first["counts"]
		merge_button.text = "⚗️ Merge (%d available)" % mergeable.size()
	else:
		merge_button.text = "⚗️ Merge (Need 3+)"
